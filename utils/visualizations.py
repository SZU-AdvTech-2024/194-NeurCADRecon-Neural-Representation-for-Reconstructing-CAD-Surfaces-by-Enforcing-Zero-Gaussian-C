...
def plot_cuts_iso(decode_points_func,
    ...
    xx = xx.ravel()     # 将二维数组xx和yy拉平成一维数组，常用于将二维平面网格转化为点云形式（即一系列点的x和y坐标）
    yy = yy.ravel()

    # 创建一个多行多列的布局，每行显示三个不同平面的可视化切片图
    ...
    def _plot_cut(fig, idx, pos, decode_points_func, xmax, ymax, resolution):       # 等值面计算与绘制
    ...
    steps = np.stack([np.linspace(-b / 2, b / 2, imgs_per_cut + 2)[1:-1] for b in box_size], axis=-1)       # 多切面绘制
